...
def readColmapSceneInfo(path, images, depths, eval, train_test_exp, model="sparse", llffhold=8):
    sparse_root = os.path.join(path, model, "0")
    try:
    ...
    if eval:
        ...
        if llffhold:
            ...
            with open(os.path.join(sparse_root, "test.txt"), 'r') as file:
